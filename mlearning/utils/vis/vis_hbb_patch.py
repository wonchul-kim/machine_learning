...
def vis_hbb_patch(img_file, idx2xyxys, idx2class, output_dir, color_map, 
                  json_dir=None, compare_gt=False, iou_threshold=0.2, line_width=2, font_scale=1):
    ...
    for cls, pred in idx2xyxys.items():
        for xyxy, conf in zip(pred['bbox'], pred['confidence']):
            ...
            cv2.putText(vis_hbb, f"{idx2class[int(cls)]} {conf:.2f}", get_text_coords(points, width, height), 
